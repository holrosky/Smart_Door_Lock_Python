...
detected = False
num_detected_device = 0
not_detected_count = 0
TIME_OUT = 3

with open('/home/pi/Smart_Door_Lock/BLE.txt','rb') as token:
  lines = token.read().split()
  for line in lines:
    BLE_devices_INFO.append(line)
    ...
    def handleDiscovery(self, dev, isNewDev, isNewData):
        ...
        for (sdid, desc, val) in dev.getScanData():
            if sdid in [8, 9]:
                ...
                print ('\t' + desc + ': <' + val + '>')
                if val in BLE_devices_INFO:
                    num_detected_device += 1
...
def main():
    ...
    parser.add_argument('-t', '--timeout', action='store', type=int, default=5,
                        help='Scan delay, 0 for continuous')
    parser.add_argument('-s', '--sensitivity', action='store', type=int, default=-75,
...
def execute():
    global detected
    global num_detected_device
    global not_detected_count
    
    while True:
        main()
        if(num_detected_device != 0):
            detected = True
            num_detected_device = 0
            not_detected_count = 0
        elif (not_detected_count < TIME_OUT) :
